...
//打开RC522驱动文件
void InitRc522Driver(void)
{
	MFRC522_cxt.fd=open("/dev/dtsrc522",O_RDWR);  
	if(MFRC522_cxt.fd==-1){  
		perror("error open\n");  
		exit(-1);  
	}  
	printf("open /dev/dtsrc522 successfully    :%8X\n",RC522_Reset);  
...
type_A 寻卡
------------------------------------------------*/
uint8_t check_RFCPU_ResetInfo(uint8_t RcvLen, uint8_t *Rcvdata ,uint8_t *CardSerialNumBuf)
{
	uint8_t i;
	uint8_t status;
...
type_A 寻卡
------------------------------------------------*/
uint8_t check_RFM1_ResetInfo(uint8_t RcvLen, void * Rcvdata,uint8_t *CardSerialNumBuf)
{
	uint8_t i;
	uint8_t status;
...
----------------------------------------------------------*/
static char TypeAB_PiccTRANSCEIVE(uint8_t SendLen)
{
	uint8_t status = MI_OK;
	ResetInfo();
...
----------------------------------------------------------*/
uint8_t ISO7816_TRANSCEIVE(uint8_t SendLen, uint8_t * SendBuf, uint8_t * RcvLen, void * Rcvdata)
{
	char  status;
	char  i;
	uint8_t TotalSendLen; //综合发送长度
	uint8_t OnceSendLen; //单次发送长度
