...
{	//bbit=0-KeyA,bbit=1--KeyB
	...
	printf_debug("CardKeyCode==%2X %2X %2X %2X %2X %2X \n",cardKeyCode[0],cardKeyCode[1],cardKeyCode[2],cardKeyCode[3],cardKeyCode[4],cardKeyCode[5]);
	printf_debug("MatchCode==%2X %2X %2X %2X  \r\n",matchCode[0],matchCode[1],matchCode[2],matchCode[3]);
	printf_debug("pMsg.CardSerialNum==%2X %2X %2X %2X \r\n",CardSerialNum[0],CardSerialNum[1],CardSerialNum[2],CardSerialNum[3]);
	printf_debug("BlockNum = %d\n",BlockNum);
	if (!bbit)//公共区用认证固定密码
...
uchar ReadCardCommonDatas_M1(void)
{
	...
	{
		//Limit_DayMoney = ChgBCDStringToUlong(rwDevMsgSt.dayLimetMoney, 3);//
		Limit_DayMoney = rwDevMsgSt.dayLimetMoney;
	} else {//按照卡里面的走
...
//	if( !Card_Rebate || Card_Rebate==0xff )	 Card_Rebate=100;
	...
	{	
		...
		{	//printf_debug("i=%d\r\n",i);
			...
			//nameBuf[0] = i;
			printf_debug("name = %s\n",RdCardDatasBuffer);
			memcpy(nameBuf,RdCardDatasBuffer,i);	
			break;
		}	
	}
	return	CARD_OK;
...
uchar ReadCardBalanceDatas_M1(uchar RdSector)//读出卡金额及余额比较
{
	...
	if (status)
		return	CARD_NOCARD;
	printf_debug("钱包号 = %d %d\n", RdSector,PursesSector[0]);
	...
	{//子钱包的有效期
		for (i=0;i<5;i++)
		{
			...
			if (!status)
			{
				...
				for(i=0;i<16;i++)
				{
					printf_debug(" %02x",RdCardDatasBuffer[i]);
				}
				printf_debug("\n");
				status=BytesCheckSum(RdCardDatasBuffer,16);
				if (!status)
	...
	for (i=0;i<5;i++)
	{
		...
		if (!status)
		{
			printf_debug("RdCardDatasBuffer +1 = ");
			for(i=0;i<16;i++)
			{
				printf_debug(" %02x",RdCardDatasBuffer[i]);
			}
	...
	for (i=0;i<5;i++)
	{
		...
		if (!status)
		{
			printf_debug("RdCardDatasBuffer +2 = ");
			for(i=0;i<16;i++)
			{
				printf_debug(" %02x",RdCardDatasBuffer[i+16]);
			}
			printf_debug("\n");
	...
	printf_debug("Money1 = %d, Money2 = %d\n",Money1,Money2);
	...
	for(i=0;i<4;i++)
	{
		printf_debug("OldBalance = %2x",OldBalance[i]);
	}
