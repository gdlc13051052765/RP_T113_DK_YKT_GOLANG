...
#define CON_SWIPE_CARD//联系刷卡测试
int consume_mode = CONSUM_MONEY;//消费模式，默认计算模式
...
int read_consume_data_form_config_db(void)
{
	...
	CurrentConsumMoney = conDevMsgSt.autoModeConsumeMoney;
	consume_mode = conDevMsgSt.consumeMode;
}
...
static void money_mode_consume_task(void)
{
	...
	{
		case 0:
		if(consume_mode == CONSUM_MONEY)//计算模式
		...
		{	
			...
			{	
				...
				break;
				default :	//
				cJSON_AddStringToObject(root, "consumeStatus", "卡片不合法"); //查询结果 
				break;	
			}
			cJSON_AddStringToObject(root, "name", nameBuf);  //用户名字
