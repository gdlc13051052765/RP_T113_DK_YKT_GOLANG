...
static char PurseEnable[] = "0304";
static int  consumeMode = 1;
static int  cardSector = 2;//公共扇区
...
char *recordDbFile = "/root/record.db";//消费记录数据库地址
//黑名单数据库
...
int sqlite3_blaknumber_open_db(void)
{
  int len;
	...
	//recordIdb 一直累加不清零
	//消费时间年/月/日
	char *sql = "create table blknumber (number int);" ;
...
int sqlite3_consume_open_db(void)
{
	...
	/* 打开或创建未采记录数据库 */
	len = sqlite3_open(recordDbFile,&recod_db);
	if( len )
	{
	   /*  fprintf函数格式化输出错误信息到指定的stderr文件流中  */
	...
	//recordIdb 一直累加不清零
	//消费时间年/月/日
	char *sql = "create table consume (tag INTEGER, id INTEGER, consumeMode INTEGER, money INTEGER, time char, cardNumber char, name char, record char);" ;
...
int sqlite3_consume_query_record_db( int number)
{
	...
	{	
		...
		{
			// 取出第0列字段的值
			recordSt.recordId = sqlite3_column_int(stmt, 1);
			// 取出第1列字段的值
			recordSt.recordTag = sqlite3_column_int(stmt, 0);
			// 取出第3列字段的值
			recordSt.CurrentConsumMoney = sqlite3_column_int(stmt, 3);
			// 取出第4列字段的值
			recordSt.ConsumeTime = sqlite3_column_int(stmt, 4);
			// 取出第7列字段的值
			//buf = malloc(sizeof(char)*100);
			char *buf = sqlite3_column_text(stmt, 7);
...
//查找最后一条交易记录
int query_last_record_index(void)
{
    ...
    recordId = 0;
	/* 打开数据库 */
	err = sqlite3_open_v2("/root/record.db", &recod_db, SQLITE_OPEN_READWRITE, NULL);
    if( err ) {
        printf_debug("Can't open database: %d\n ", sqlite3_errmsg(recod_db));
        sqlite3_close_v2(recod_db);
        return err;
    }
	else 
		printf_debug("You have opened a sqlite3 database named user successfully");
    
    //查找最后一条交易记录的id号
    char *idSql = "select * from consume order by id desc limit 1";//查找最后一条交易记录的ID号
	if (sqlite3_prepare_v2(recod_db, idSql, -1, &stmt, NULL) == SQLITE_OK) 
	{   
		while(sqlite3_step(stmt) == SQLITE_ROW ) 
		{	
			// 取出第1列字段的值
