...
/*=======================================================================================
* 函 数 名： message_queue_data_parsing
* 参    数： 消息队列id，
* 功能描述:  消息队列数据解析
* 返 回 值： 成功返回0
* 备    注： 
...
* 创建时间： 2021-04-25 
==========================================================================================*/
extern int consume_mode;
static void message_queue_data_parsing(mymesg_t ckxmsg)
{
  C_DevMsgSt rwDevMsgSt;//设备信息
  switch(ckxmsg.mtext[1])
  {
    case QT_CURRENTMONEY_CMD://QT发送消费额到golang
      memcpy(&CurrentConsumMoney,&ckxmsg.mtext[2],4);
      
      printf_debug("QT发送过来的消费额 = %d\n", CurrentConsumMoney);
    break;

    case QT_CHANGE_CONSUME_MODE_CMD://QT发送过来更改消费模式
      printf_debug("QT发送过来的更改消费模式 = %d\n", ckxmsg.mtext[2]);
      if(ckxmsg.mtext[2] !=  consume_mode)
      {
        consume_mode = ckxmsg.mtext[2];
        if(consume_mode == 1)//自动模式
        {
          rwDevMsgSt = sqlite_read_devMsg_from_config_db();
          CurrentConsumMoney = rwDevMsgSt.autoModeConsumeMoney;//自动模式消费金额
        }
    ...
    case QT_CANCEL_CONSUME_CMD://QT发送消费取消
      ConsumCase = 0;//消费状态机复位
      printf_debug("消费取消\n");
    break;
  }
}
...
int receive_qt_msg_data_task(void)
{
  ...
  if(rec_len < 0) {
   ...
   // printf_debug("ckxmsg.type = %02x \n",ckxmsg.type);
    ...
    printf_debug("\n");
    message_queue_data_parsing(ckxmsg);//消息队列数据解析
    MSGTASKDELAY = 5000;//队列任务延时设置成10ms 
  }
}
