...
{
	return check_RFCPU_ResetInfo(CPU_RcLen, CPU_CommBuf, CardSerialNum);//卡复位信息
}
...
uchar Detect_Card(void)
{
	...
	status=check_RFCPU_ResetInfo(CPU_RcLen, CPU_CommBuf, CardSerialNum);//卡复位信息
	if(status)
		status=CARD_NOCARD;
	else
	{
